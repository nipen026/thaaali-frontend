...
function ChannelBadge({order}){
  if(order.table_id) return <>Table {order.table_number}</>;
...
export default function KDSPage(){
  ...
  useEffect(()=>{
    if(!socket)return;
    socket.on('new_order',o=>{setOrders(p=>[o,...(p||[])]);notifyInfo('New order!');});
  ...
  const upd=async(id,status)=>{
    ...
    notifySuccess(status==='ready'?'Order ready!':status==='delivered'?'Delivered!':'Updated');
  };
    ...
    <div>
      <div className="flex-between" style={{marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div>
          <div className="flex gap-3" style={{marginBottom:2}}>
            <span style={{fontFamily:'var(--font-d)',fontSize:20,fontWeight:800}}>Kitchen Display</span>
            {orders.length>0&&(
          ...
          <div style={{fontSize:12.5,color:'var(--muted)'}}>Real-time · Auto-updates every minute</div>
        </div>
        <div className="flex gap-2" style={{flexWrap:'wrap'}}>
          ...
          <button className="btn btn-sc btn-sm" onClick={load}><RefreshCw size={13}/> Refresh</button>
        </div>
      </div>
      ...
      {orders.length===0?(
        <EmptyState
          ...
          subtitle="No active orders. Enjoy the calm before the storm."
        />
      ):(
                  ...
                  <div className="kds-hd" style={{background:hdBg[order.status]}}>
                    <div>
                      ...
                      <div style={{fontSize:11,color:'var(--muted)',marginTop:2}}>
                        {order.items.length} items · #{order.id.slice(0,6).toUpperCase()}
                      </div>
                    </div>
                      ...
                      <div key={i} className="kds-row">
                        ...
                        <div style={{flex:1}}>
                          ...
                          {(item.notes?.toLowerCase().includes('jain')||item.dietary?.includes('jain'))&&(
                            <div className="kds-allergen-tag flex gap-1"><AlertTriangle size={11}/> JAIN — No onion/garlic/root veg</div>
                          )}
                        </div>
                  ...
                  <div className="kds-actions">
                    {order.status==='pending'&&(
                      <motion.button className="btn btn-sm"
                        ...
                        onClick={()=>upd(order.id,'preparing')}><Flame size={14}/> Start Cooking</motion.button>
                    )}
                    {order.status==='preparing'&&(
                      <motion.button className="btn btn-su btn-sm"
                        style={{flex:1,justifyContent:'center'}}
                        whileHover={{scale:1.02}} whileTap={{scale:.96}}
                        onClick={()=>upd(order.id,'ready')}><CheckCircle size={14}/> Mark Ready</motion.button>
                    )}
                    {order.status==='ready'&&(
                      <motion.button className="btn btn-sm"
                        style={{flex:1,justifyContent:'center',background:'var(--jade)',color:'white',border:'none'}}
                        whileHover={{scale:1.02}} whileTap={{scale:.96}}
                        onClick={()=>upd(order.id,'delivered')}><Check size={14}/> Delivered</motion.button>
                    )}
                  </div>
                </motion.div>
              );
