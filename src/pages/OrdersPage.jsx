  ...
  return(
    <div>
      <div className="flex-between" style={{marginBottom:16,flexWrap:'wrap',gap:12}}>
        <div style={{fontSize:13,color:'var(--muted)'}}>{orders.filter(o=>['pending','preparing','ready'].includes(o.status)).length} active · {orders.length} total</div>
      </div>
      <div className="filter-bar">
        {['active','all','pending','preparing','ready','delivered','billed'].map(s=>(
          <button key={s} className={`chip${filter===s?' on':''}`} onClick={()=>setFilter(s)}>{s.charAt(0).toUpperCase()+s.slice(1)}</button>
        ))}
      </div>
      <div className="stack gap-2">
              ...
              <motion.div key={o.id} className="card"
                ...
                <div className="flex gap-4" style={{padding:'13px 20px',flexWrap:'wrap'}}>
                  ...
                  <div style={{flex:1,minWidth:120}}>
                    <div style={{fontSize:13.5,fontWeight:700}}>{o.table_id?`Table ${o.table_number}`:(o.channel||'Order')}</div>
                    <div style={{fontSize:11.5,color:'var(--muted)'}}>#{o.id.slice(0,8).toUpperCase()} · {o.items.length} items</div>
                  </div>
                  <div className="flex gap-1" style={{flexWrap:'wrap'}}>
                    ...
                    {o.items.length>3&&<span style={{fontSize:12,color:'var(--muted)'}}>+{o.items.length-3} more</span>}
                  </div>
                  <div style={{textAlign:'right',minWidth:80}}>
                    <div style={{fontFamily:'var(--font-d)',fontWeight:800,color:'var(--saffron)'}}>₹{o.total}</div>
                    <span className={`badge ${SB[o.status]||'bg-gray'}`} style={{marginTop:4}}>{o.status}</span>
                  </div>
                </div>
              </motion.div>
            );
        ...
        {filtered.length===0&&(
          <EmptyState icon={<ClipboardList size={48} strokeWidth={1.5}/>} title={`No ${filter} orders`}/>
        )}
      </div>
    </div>
  );
